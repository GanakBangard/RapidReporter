            ...
            if (!String.IsNullOrWhiteSpace(textBoxContents))
            {
                ...
                try
                {
                    if (!Directory.Exists(WorkingDir))
                    {
                        ...
                        Logger.Record("\t[save_Click]: WorkingDir created: " + WorkingDir, "PlainTextNote", "info");
                    }
                }
                catch (Exception ex)
                {
                    Logger.Record("\t[save_Click]: Could not create WorkingDir: " + ex.Message, "PlainTextNote", "error");
                    System.Windows.MessageBox.Show("Cannot create the directory to save notes. Please check permissions.\n" + ex.Message, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                ...
                try
                {
                    string testFile = Path.Combine(WorkingDir, "__rr_write_test.txt");
                    File.WriteAllText(testFile, "test");
                    File.Delete(testFile);
                }
                catch (Exception ex)
                {
                    Logger.Record("\t[save_Click]: Directory not writable: " + ex.Message, "PlainTextNote", "error");
                    System.Windows.MessageBox.Show("Cannot write to the directory for notes. Please choose a different folder or check permissions.\n" + ex.Message, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
